...
function updateLabelNode(node, track){
  ...
  const verticalTrend = formatVsIndicator(track.verticalSpeed);
  if(verticalTrend){
    node.levelsDisplay.dataset.trend = verticalTrend;
  }else{
    delete node.levelsDisplay.dataset.trend;
  }
  ...
  const muteLevels = !verticalTrend && !hasDisplayLevels;
  node.levels.classList.toggle('muted', muteLevels);
  const muteLevelsDisplay = muteLevels;
  node.levelsDisplay.classList.toggle('muted', muteLevelsDisplay);
