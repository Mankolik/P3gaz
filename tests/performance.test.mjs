...
import {selectFlightLevel,FLIGHT_LEVELS} from '../src/radar/spawner.js';
...
test('every type actually climbs and descends at its supplied phase rates',()=>{
  for(const type of Object.keys(AIRCRAFT_PERFORMANCE))for(const [level,target,phase,sign] of [
    ...
    const t=track(type,level,target);advance(t,5);
...
test('profile changes at altitude boundaries and reaches a clearance without overshoot',()=>{
  const t=track('A320',49.9,155);advance(t,2);
...
test('vertical assignments respect type capability and level capture',()=>{
  for(const [comparator,value,expected] of [['exact',900,900],['or-less',1000,1000],['or-greater',1000,2500],['exact',4000,2500]]){
    const t=track('A320',10,100);t.verticalRateAssigned=true;t.assignedVertical={comparator,value};advance(t,5);
...
test('directional spawn levels and subsequent climbs never exceed each type ceiling',()=>{
  for(const profile of Object.values(AIRCRAFT_PERFORMANCE))for(const heading of [0,90,180,270])for(const roll of [0,0.25,0.5,0.99,1]){
    const level=selectFlightLevel(heading,()=>roll,profile.ceilingFL);
    assert(level<=profile.ceilingFL);assert(FLIGHT_LEVELS[heading<180?'east':'west'].includes(level));
  }
  assert.throws(()=>selectFlightLevel(0,()=>0,100),/No directional/);
  for(const type of Object.keys(AIRCRAFT_PERFORMANCE)){
