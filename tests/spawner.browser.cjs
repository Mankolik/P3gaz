...
(async()=>{
  ...
  try{
    ...
    assert.equal(track.expectedCruiseLevel,180);assert.equal(track.exitFlightLevel,280);
    ...
    assert.equal(await departureLabel.locator('.level-exit button').textContent(),'280');
    ...
    assert.equal(await departureLabel.locator('.level-exit button').textContent(),'280','editing ECL must not replace XFL');
    ...
    await page.evaluate(()=>{
      ...
      const groups=state.air.spawner.catalogue.groups;window.spawnDraws=[(groups.findIndex(g=>g.departure==='KJFK'&&g.destination==='EPWA')+0.1)/groups.length,0,0,0.75,0.5,0.5];
    });
    ...
    assert.equal(track.boxExitLevel,true);assert.equal(track.exitFlightLevel,track.clearedFlightLevel);
    ...
    assert.equal(await xfl.textContent(),String(track.exitFlightLevel));
    assert.equal(await xfl.evaluate(el=>getComputedStyle(el).opacity),'1','boxed repeated XFL remains visible without hover');
